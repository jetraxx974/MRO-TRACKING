...
def save_user(email, password, first_name, last_name, company):
    ...
    data = {"email": email, "password": hashed_pw, "first_name": first_name, "last_name": last_name, "company": company, "Status": False}
    try:
...
def run_mro_app():
    ...
    # Initialize Display Cols default if not present
    ...
    if st.session_state['current_view'] == "Visualization":
        with st.expander("⚙️ Filter & Column Configuration", expanded=True):
            ...
            master_filter_cols = c2.multiselect(
                ...
                default=st.session_state['visu_saved_master_cols'],
            ...
            user_selection = c3.multiselect(
                ...
                default=st.session_state['visu_saved_display_cols'],
                key="visu_columns_select"
            )
        ...
        with c_time:
            period = st.radio(
                ...
                index=["View All", "7 Days", "30 Days", "60 Days", "180 Days"].index(st.session_state['visu_saved_period']),
                horizontal=True, 
                key="period_radio"
            )
    ...
    # --- VIEW 2: SCHEDULE & EDIT ---
        ...
        with col_form:
            ...
            with st.form("job_form", border=True):
                ...
                btn_txt = "💾 Update" if st.session_state['edit_mode'] else "💾 Save Schedule"
    ...
    # --- VIEW 3: FOLDERS ---
                ...
                if c_sub.form_submit_button("Create"):
                    if new_fname:
                        if create_folder(new_fname, st.session_state['user_email']):
                            st.success("Folder created!"); st.rerun()
        ...
        for folder in filtered_folders:
            ...
            if search_query: jobs_in = [j for j in jobs_in if search_query.lower() in j['task_name'].lower() or search_query.lower() in fname.lower()]
            ...
            with st.expander(f"📁 **{fname}** ({len(jobs_in)})", expanded=(True if search_query else False)):
                ...
                if c_btn.button("💾 Rename", key=f"bsave_{fid}"):
                    if new_name != fname: rename_folder(fid, new_name); st.rerun()
                
                with c_del_f:
                    with st.popover("🗑️ Folder"):
                ...
                if not jobs_in: st.caption("No reports.")
                    ...
                    for j in jobs_in:
                        ...
                        with c2:
                            if st.button("✏️ Edit", key=f"fold_edit_{jid}"):
                                st.session_state['edit_mode'] = True
...
def load_stored_data(user_email):
    try:
        ...
        start = 0
        while True:
            res = supabase.table("raw_data_table").select("row_data").eq("owner_email", user_email).range(start, start + 9999).execute()
            ...
            if choice == "Login":
                with st.form("login"):
                    ...
                    if st.form_submit_button("Connect"):
                        u = login_user(e, p)
                        if u:
                            st.session_state.update({"logged_in": True, "user_email": u['email'], "user_first_name": u['first_name'], "user_last_name": u['last_name'], "user_company": u['company']})
                            st.rerun()
                        else: st.error("Error")
            else:
                with st.form("signup"):
                    ...
                    if st.form_submit_button("Create"):
                        if em and pw:
                            if save_user(em, pw, fn, ln, cp): st.success("Created! Log in.")
    else: run_mro_app()
